...
import { NAVIGATION, UserProfileLink } from '@/components/dashboard/mobile-header';
import { NavItem } from '@/components/dashboard/nav-item';
import { AnimatedButton, CreatePostModal } from '@/components/shared';
    ...
    <>
      <aside className="sticky top-0 flex h-screen w-full flex-col px-4 py-6">
        ...
        <div className="my-6 space-y-2">
          <AnimatedButton onClick={() => setShowCreateModal(true)} icon={<PenSquare className="h-4 w-4" />}>
        ...
        <div className="border-sidebar-border border-t pt-4">
          <UserProfileLink lang={lang} />
        </div>
      </aside>
      <CreatePostModal open={showCreateModal} dictionary={dictionary} onOpenChange={setShowCreateModal} />
    </>
  );
}
